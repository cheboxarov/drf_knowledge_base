...
class Article(models.Model):
    ...
    section = models.ForeignKey(
        "sections.Section", on_delete=models.CASCADE, null=False
    )
    ...
    def save(self, *args, **kwargs):
        with transaction.atomic():
            if not self.pk:
                ...
                max_position = Article.objects.filter(section=self.section).aggregate(
                    Max("position")
                )["position__max"]
    ...
    def delete(self, *args, **kwargs):
        with transaction.atomic():
            # Сдвигаем статьи вверх в рамках секции
            Article.objects.filter(
                section=self.section, position__gt=self.position
            ).update(position=F("position") - 1)
