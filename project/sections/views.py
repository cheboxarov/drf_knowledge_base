...
class SectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            return (
                Section.objects.filter(project=user.project)
                .order_by("position")
                .prefetch_related("project")
            )
        return (
            Section.objects.filter(Q(id__in=user.view_list) & Q(project=user.project))
            .order_by("position")
            .prefetch_related("project")
        )
    ...
    def perform_create(self, serializer):
        if not self.request.user.is_staff:
            raise PermissionDenied(
                detail="You do not have permission to create this section."
            )
