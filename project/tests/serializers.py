        ...
        for question in value:
            ...
            if question.get("type") not in ["single", "multiple"]:
                raise ValidationError(
                    'Type is required and must be either "single" or "multiple".'
                )
            if not isinstance(question.get("question"), str):
                raise ValidationError("Question is required and must be a string.")
            if not isinstance(question.get("answers"), list) or not all(
                isinstance(answer, str) for answer in question.get("answers")
            ):
                raise ValidationError(
                    "Answers is required and must be an array of strings."
                )
            ...
            if (
                ...
                and len(question.get("right_answers")) != 1
            ):
                raise ValidationError(
                    "If th type is single, then only 1 correct answer is required"
                )
            for right_answer in question.get("right_answers"):
                if right_answer not in question.get("answers"):
                    raise ValidationError(
                        {
                            right_answer: "The correct answer must be contained in the answers"
                        }
                    )
        return value
